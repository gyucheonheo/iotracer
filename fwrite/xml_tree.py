...
#                leaf = ET.SubElement(parent, func_name[0])
#                leaf.set('time', str(exec_time[0]))
                i+=1
            elif (len(func_end) == 1 ):
                parent["value"] = float(exec_time[0])
#                parent.set("duration", exec_time[0])
                return i+1
            else:
                i+=1
                continue
