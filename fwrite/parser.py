...
def exportCSV(inf):
    ...
    with open('fwrite_t.csv', mode='w') as data_file:
    ...
    for flag, dic in inf.items():
        ...
        with open('fwrite_t.csv', mode='a') as data_file:
            data_writer = csv.writer(data_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    ...
    for d in dir_list:
        ...
        for i in range (1,dir_count+1):
            ...
            for line in data:
                ...
                """
                if len(func_name) > 0 and func_name[0] == "SyS_write()":
                    SyS_count += 1

                if "SyS_write()" in inf and inf["SyS_write()"][1] == i and SyS_count > 1:
                    break
